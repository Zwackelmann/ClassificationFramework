...
object Test5 {
    def main(args: Array[String]) {
        println("something")
        val trainset = new ArffJsonInstancesFile(ContentDescription("final-mini", ContentDescription.TrainSet, List()))
        val testset = new ArffJsonInstancesFile(ContentDescription("final-mini", ContentDescription.TestSet, List()))
        
        val projectionFilter = new ProjectionFilter(List(2), HistoryItem("jour"))
