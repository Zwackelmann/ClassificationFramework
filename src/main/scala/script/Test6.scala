...
object Test6 {
    def main(args: Array[String]) = try {
        ...
        common.Path.rootFolder = "data_test"
        ...
        // Eine neue ArffJsonInstancesSource erstellen, die nur die Dokumente aus der Zielkategorie enthält
        val documentsInTargetCategory = ArffJsonInstancesSource(
            corpus.filter(_.categories.exists(_ == mscString)).toList,      // Das "toList" hier sorgt dafür, dass die Dokumente tatsächlich alle in den Arbeitsspeicher geladen werden.
                                                                            // Das ist sinnvoll, wenn nur auf einem Teil des Korpus gearbeitet wird - der gesammte Korpus passt
                                                                            // nämlich nicht in einen normal großen RAM.
            corpus.header
        )
        
        // Die Dokumente in den Vektorraum des Learners überführen.
